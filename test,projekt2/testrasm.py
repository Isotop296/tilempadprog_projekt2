...
def audio_in():
    ...
    # Output: 
        ...
        Return 2: None - UnknownValueError
...
def is_wmp_running():
    ...
    # Here is the main loop of the program which ends if the user inputs "n". (again == n)
while True:
    if is_wmp_running():
        subprocess.call("taskkill /IM wmplayer.exe /F", shell=True)

    print("Please wait for the listening prompt.")
    ...
    again = input("Do you want to continue chatting? (y/n)")
    if again == 'n':
        subprocess.call("taskkill /IM wmplayer.exe /F", shell=True)
        break